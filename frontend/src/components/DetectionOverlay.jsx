      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-9 gap-2">
        {Object.entries(CLASS_META).map(([key, meta]) => {
          ...
          return (
            <div
              key={key}
              className={`border rounded-xl p-3 flex flex-col items-center gap-1 transition-colors ${cardColor(count)}`}
            >
              <span className="text-2xl">{meta.icon}</span>
              <span className="text-xs text-gray-400 text-center leading-tight">{meta.label}</span>
              <span className={`text-xl font-bold ${countColor(count)}`}>{count}</span>
            </div>
          )
        })}
      </div>
